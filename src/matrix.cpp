...
// #####################################################
    ...
    Matrix getRotationMatrix(double roll, double pitch, double yaw) {
        double vRoll[9] = {1, 0, 0, 0, cos(roll), -sin(roll), 0, sin(roll), cos(roll)};
        double vPitch[9] = {cos(pitch), 0, sin(pitch), 0, 1, 0, -sin(pitch), 0, cos(pitch)};
        double vYaw[9] = {cos(yaw), -sin(yaw), 0, sin(yaw), cos(yaw), 0, 0, 0, 1};

        Matrix rollMat(3, 3, vRoll);
        Matrix pitchMat(3, 3, vPitch);
        Matrix yawMat(3, 3, vYaw);

        Matrix resultYP = multiply(yawMat, pitchMat);
        return multiply(resultYP, rollMat);
    }
}  // namespace Matrix
