...
void Flash::saveToFlash() {
    eraseSector(sector);
    writeSector(sector, &data, sizeof(data));
}

void Flash::loadFromFlash() {
    readSector(sector, &data, sizeof(data));
}

int Flash::writeSector(uint32_t Address, void *values, uint16_t size) {
    uint16_t *AddressPtr;
    uint16_t *valuePtr;
...
void Flash::readSector(uint32_t SectorStartAddress, void *values, uint16_t size) {
    uint16_t *AddressPtr;
    uint16_t *valuePtr;
...
void Flash::eraseSector(uint32_t SectorStartAddress) {
    FLASH->KEYR = 0x45670123;
    FLASH->KEYR = 0xCDEF89AB;
